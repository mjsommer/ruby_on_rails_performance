...
- Ruby 3.3.12, Rails 8.1.3.1 (`config.load_defaults 8.1`, fully adopted — no lingering `new_framework_defaults_*.rb` initializers). Bundler is pinned to `2.5.22` in `Gemfile.lock` — that's Ruby 3.3.12's own default, not an arbitrary choice (see the Bundler/RubyGems gotcha below)
...
- **Bundler/RubyGems gotcha**: `Gemfile.lock`'s `BUNDLED WITH` must track whatever Bundler version ships by default with the pinned Ruby (`2.5.22` for Ruby 3.3.12). Ordinary top-level `bundle exec` commands tolerate a mismatch (an older pinned Bundler self-installs fine under a newer Ruby's RubyGems), but `bin/ci` shells out to a *nested* `bundle exec` subprocess, and that path breaks with a `LoadError` (a duplicated gem path segment) plus `already initialized constant Gem::Platform::*` warnings if the two are out of sync. First discovered during the Ruby 3.3.12 → 4.0.7 upgrade, when `BUNDLED WITH` was left at the old `2.5.22` — fixed by running `bundle _<version>_ install` to relock to the Ruby-default Bundler. Re-check this any time Ruby's version changes, even if plain `bundle exec` commands still seem to work.
- **Ruby 4.0.7 → 3.3.12 reversion (2026-09-24)**: after the Rails 8.1 upgrade above landed on Ruby 4.0.7, the app was deliberately downgraded back to Ruby 3.3.12 to set up an A/B performance comparison between Ruby 3.x and Ruby 4.x on this codebase — not a correctness or compatibility problem with 4.0.7. All 102 resolved gems were checked and none require Ruby >= 4.0 (the tightest constraints, `>= 3.3` on `rbs`/`selenium-webdriver`/`simplecov`, are satisfied), so no gem versions changed — only `.ruby-version`, `Gemfile`'s `ruby` directive, and `Gemfile.lock`'s `RUBY VERSION`/`BUNDLED WITH` moved. `bin/ci` passes clean under 3.3.12. If Ruby is bumped back to 4.0.7 (or newer) later, re-apply the same `BUNDLED WITH` relock step above.
